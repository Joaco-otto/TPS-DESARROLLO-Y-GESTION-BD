-- TP PROCEDIMIENTOS JOAQUIN FLORES
...
END //
DELIMITER ;
-- LLAMAMOS AL PROCEDIMIENTO CREADO ANTERIORMENTE
CALL total_ventas_por_producto();
-- EJERCICIO 2

-- 2. Procedimiento que actualiza el stock de un producto y devuelve su nuevo valor
DELIMITER //
CREATE PROCEDURE actualizar_stock(IN p_id INT, IN nueva_cantidad INT)
BEGIN
    UPDATE productos
    SET stock = nueva_cantidad
    WHERE id = p_id;

    SELECT nombre, stock AS nuevo_stock
    FROM productos
    WHERE id = p_id;
END //
DELIMITER ;
...
-- EJERCICIO 3
-- Productos con stock menor a cierto valor
DELIMITER //
...
DELIMITER ;
-- LLAMAMOS AL PROCEDIMIENTO CREADO ANTERIORMENTE
CALL productos_stock_bajo(10);

-- EJERCICIO 4
-- Procedimiento que muestra el nombre y la cantidad de compras de un cliente en un rango de fechas
DELIMITER //
...
END //
DELIMITER ;
-- llamamos al procedimiento creao anteriormente
CALL compras_cliente_rango(1, '2022-01-01', '2022-12-31');

-- EJERCICIO 5
-- Procedimiento que muestra el promedio de precio de los productos comprados por un cliente
DELIMITER //
CREATE PROCEDURE promedio_precio_cliente(IN p_cliente_id INT)
BEGIN
    SELECT c.nombre AS cliente,
...
-- llamamos al procedimiento creao anteriormente
CALL promedio_precio_cliente(2);

-- EJERCICIO 6
-- Procedimiento que muestra la lista de clientes que han comprado un producto en particular
DELIMITER //
...
-- EJERCICIO 7
-- Procedimiento que actualiza el precio de un producto y devuelve su nuevo valor
DELIMITER //
CREATE PROCEDURE actualizar_precio(IN p_id INT, IN nuevo_precio DECIMAL(10,2))
BEGIN
    UPDATE productos
    SET precio = nuevo_precio
    WHERE id = p_id;

    SELECT nombre, precio AS nuevo_precio
    FROM productos
    WHERE id = p_id;
END //
DELIMITER ;
-- llamamos al procedimiento creao anteriormente
CALL actualizar_precio(2, 950.00);

-- EJERCICIO 8
-- Procedimiento que muestra el nombre y el stock de los productos que se han vendido en un rango de fechas
...
DELIMITER ;
-- llamamos al procedimiento creao anteriormente 
CALL productos_vendidos_rango('2022-01-01', '2022-01-10');

-- Procedimiento que muestra el total de ventas por cliente
DELIMITER //
...
-- Procedimiento que muestra la lista de productos ordenada por precio de mayor a menor
